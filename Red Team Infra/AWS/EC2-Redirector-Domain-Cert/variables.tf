...
variable "cloudflare_api_token" {
  ...
  validation {
    condition     = length(trim(var.cloudflare_api_token, " \t\n\r")) > 0
    error_message = "The cloudflare_api_token variable must not be empty."
  }
}

variable "cloudflare_zone_name" {
  type        = string
  description = "Your domain name, e.g., 100daysofredteam.com"
